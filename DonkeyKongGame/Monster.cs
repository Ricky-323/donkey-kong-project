...
namespace DonkeyKongGame
{
    public class Monster
    {
        ...
        public Monster(float startX, float startY, float minX, float maxX)
        {
            X = startX;
            Y = startY;
            _minX = minX;
        ...
        private void LoadImages()
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                string path = Path.Combine(monsterDir, $"MushroomRun{i + 1}.png");
            ...
            _attackFrames = new Image[10];
            for (int i = 0; i < 10; i++)
            {
                string path = Path.Combine(monsterDir, $"MushrromAttack{i + 1}.png");
